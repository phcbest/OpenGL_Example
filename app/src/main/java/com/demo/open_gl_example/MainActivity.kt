...
@Composable
private fun GlImagePickerWithGesture() {
    val context = LocalContext.current
    val pickerLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            context.startActivity(
                Intent(context, ImageGlDisplayGestureActivity::class.java).putExtra(EXTRA_IMAGE_URI, uri.toString())
            )
        }
    }
    Button(
...
private fun GlImagePicker() {
    ...
    val pickerLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            context.startActivity(
                Intent(context, ImageGlDisplayActivity::class.java).putExtra(EXTRA_IMAGE_URI, uri.toString())
            )
        }
    }
    Button(
